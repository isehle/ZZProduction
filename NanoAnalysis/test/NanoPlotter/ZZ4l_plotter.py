...
from H4l_draw_mZZ_full2022 import getZX
...
class ZZHists:
    def __init__(self):
        ...
        self._cand_maps = dict(
                            ...
                            Z1 = dict(
                                SR = dict(
                                    idx = "bestCandIdx",
                                    col = lambda prop: "ZZCand_Z1{}".format(prop)
                                )
                            ),
                            Z2 = dict(
                                SR = dict(
                                    idx = "bestCandIdx",
                                    col = lambda prop: "ZZCand_Z2{}".format(prop)
                                )
                            )
                        )
    ...
    def _fill_hist(self, fname, proc, info):
        ...
        z_lep_ids = df.AsNumpy(["ZZCand_Z1l1Idx", "ZZCand_Z1l2Idx", "ZZCand_Z2l1Idx", "ZZCand_Z2l2Idx"])
        
        z1_el = df.Filter("ZZCand_Z1flav==-121") # This leads to a segfault when creating hist #however Events->Draw("Electron_pt", "ZZCand_Z1flav==-121") works as expected
        breakpoint()
        z1_el  = df.Filter("Sum(ZZCand_Z1flav==-121)") # This works, the above leads to an array of bools
        z1_two_el = z1_el.Filter("nElectron==2")
        z1_twoEl_hist = z1_two_el.Histo1D("Electron_pt").GetValue()
        breakpoint()
        two_el = df.Filter("nElectron==2")
    ...
    def fillHistos(self, **kwargs):
        ...
        for cat, cat_dict in kwargs["proc_info"].items():
            ...
            for proc_dict in cat_dict["eras"].values():
                ...
                samples_by_cat = {cat: [sample[1] for sample in samples]}

                Hist = self._fill_hist(samples_by_cat[cat], cat, kwargs["hist_info"])
    ...
    def runZZ(self, **kwargs):
        ...
        if "Z+X" in kwargs["proc_info"]:
            hists.update({
                ...
                "Z+X": getZX(hists["H(125)"])
            })
            hists["Z+X"].Scale(kwargs["proc_info"]["Z+X"]["eras"]["2022_EE"]["lum"])

        # Explicit ordering for comparison
        '''self.hists = {
            "Z+X": hists["Z+X"],
            "EW": hists["EW"],
            "gg#rightarrow ZZ,Z#gamma*": hists["gg#rightarrow ZZ,Z#gamma*"],
            "q#bar{q}#rightarrow ZZ,Z#gamma*": hists["q#bar{q}#rightarrow ZZ,Z#gamma*"],
            "H(125)": hists["H(125)"],
            "Data": hists["Data"]
        }'''
...
class ZZPlotter:
    ...
    def _get_hStacks(self, **kwargs):
        ...
        HStack = ROOT.THStack("Stack_{}{}".format(kwargs["hist_info"]["which"], kwargs["hist_info"]["prop"]),
                          "; {} ; {}".format(kwargs["plot_info"]["x_title"], kwargs["plot_info"]["y_title"]))

        for proc, h in self.zz.hists.items():
            h.GetXaxis().SetTitle(kwargs["plot_info"]["x_title"])
            h.GetYaxis().SetTitle(kwargs["plot_info"]["y_title"])
    ...
    def _set_lumi(self, Canvas, **kwargs):
        ...
        CMS_lumi.lumi_sqrtS           = "{} fb-1 (13.6 TeV)".format(round(kwargs["lumin"]*1e-3, 1))
    ...
    def _save_plot(self, Canvas, **kwargs):
        ...
        reg_direc = os.path.join(date_direc, kwargs["hist_info"]["reg"])
        os.makedirs(reg_direc, exist_ok=True)

        axes = self._get_axes(Canvas, **kwargs)
        plot_path = "{}_{}.{}".format(
            kwargs["plot_info"]["out_file"],
            axes,
            kwargs["plot_info"]["format"]
        )
